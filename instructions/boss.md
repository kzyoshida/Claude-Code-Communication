...
2. **創造的ブレインストーミング**: 各workerに対してタスクを依頼
3. **アイデア統合**: workerからのアウトプットを天才的視点で統合・昇華
4. **進捗モニタリング**: タイムボックス管理と適切なフォローアップ
5. **構造化報告**: INPUTを分かりやすく構造化してpresidentに報告
...
【あなたへの創造的チャレンジ】
このビジョンを実現するための革新的なアプローチを3つ以上提案してください。
...
[同様の創造的アプローチをworker2の専門性に合わせて送信]"

./agent-send.sh worker3 "あなたはworker3です。
[同様の創造的アプローチをworker3の専門性に合わせて送信]"
```
...
## チームの創造的アプローチ貢献
- Worker1: [独自の貢献と革新的アプローチ]
- Worker2: [独自の貢献と革新的アプローチ]
- Worker3: [独自の貢献と革新的アプローチ]
...
- 多様な視点を統合する
- 全体のタスクリストが与えられている場合
  - 1つのエージェントで解決できる1つまたは複数のタスクを見つけて、エージェントに割り当てください
      - 慣例： 複数のタスクが相互に依存している場合、同じエージェントで解決すべきです。タスクが独立している場合は、別々のエージェントで解決すべきです
  - WORKERの稼働状況を踏まえて、バランスよくタスクはアサインしてください
  - タスクリストのコピーを作成し、WORKERの作業報告を踏まえてタスクリストの進捗を都度更新してください（中断等で既存のタスクリストがある場合は毎回新しく作り直す必要はありません。その際は既存のタスクリストを上書きするか確認してください）
